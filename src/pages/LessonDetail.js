...
function LessonDetail() {
  ...
  useEffect(() => {
    const fetchLesson = async () => {
      try {
        const response = await axios.get(`https://artem.diplomcomtehno.online/api/lessons/${lessonId}/`);
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      try {
        ...
        const response = await axios.get('https://artem.diplomcomtehno.online/api/progress/', config);
  ...
  const handleCompleteLesson = async () => {
    try {
      ...
      if (!pid) {
        ...
        const createResp = await axios.post('https://artem.diplomcomtehno.online/api/progress/', { course_id: Number(courseId) }, config);
        pid = createResp.data.id;
        setProgressId(pid);
      }
      // Отметить урок как пройденный
      await axios.post(`https://artem.diplomcomtehno.online/api/progress/${pid}/complete_lesson/`, { lesson_id: lessonId }, config);
